...
async function loadHistorial() {
  ...
  if (SPREADSHEET_ID.includes('TU_SPREADSHEET_ID')) {
    el.innerHTML = `<div class="empty-state"><div class="empty-state-icon">⚙️</div><p>Configurá el Spreadsheet ID primero</p></div>`;
    return;
  }
  ...
  try {
    ...
    if (!historial.length) {
      el.innerHTML = `<div class="empty-state"><div class="empty-state-icon">📋</div><p>Sin actividad registrada todavía</p></div>`;
    } else {
      el.innerHTML = historial.slice().reverse().map(h => `
        <div class="prestamo-item" style="cursor:default">
          <div>
            <div class="prestamo-nombre">${h.ClienteNombre}</div>
            <div class="prestamo-meta">${h.Accion}</div>
          </div>
          <div class="prestamo-meta" style="text-align:right;flex-shrink:0">${h.Fecha}</div>
        </div>`).join('');
    }
  } catch(e) { toast('Error cargando historial: ' + e.message, 'error'); }
  setLoading(false);
}
...
async function loadDashboard() {
  ...
  try {
    ...
    // Próximas a vencer y gráfico, en paralelo, sin bloquear el resto
    cargarProximasAVencer();
...
async function cargarProximasAVencer() {
  ...
  if (!el) return;
  try {
    const proximas = await getProximasAVencer(7);
    if (!proximas.length) {
      el.innerHTML = `<div class="empty-state" style="padding:1.25rem"><p>Sin cuotas por vencer en 7 días</p></div>`;
      return;
    }
    const fmt = n => '$' + parseFloat(n || 0).toLocaleString('es-AR', { minimumFractionDigits: 0 });
    el.innerHTML = proximas.map(c => {
      const prestamo = dashData?.prestamos.find(p => p.ID === c.PrestamoID);
      const nombre   = prestamo?.ClienteNombre || '—';
      return `
        <div class="prestamo-item" onclick="${prestamo ? `verDetallePrestamo('${c.PrestamoID}')` : ''}">
          <div>
            <div class="prestamo-nombre">${nombre}</div>
            <div class="prestamo-meta">Cuota ${c.NroCuota} · Vence ${c.FechaVencimiento}</div>
          </div>
          <div class="prestamo-monto">
            <div class="prestamo-deuda" style="color:var(--accent)">${fmt(c.MontoTotal)}</div>
          </div>
        </div>`;
    }).join('');
  } catch(e) { console.error('Error cargando próximas a vencer:', e); }
}

async function cargarGraficoGanancias() {
  const el = document.getElementById('grafico-ganancias');
  if (!el) return;
  try {
    const datos = await getGananciaPorMes();
    if (!datos.length) {
      el.innerHTML = `<div class="empty-state" style="padding:1.25rem"><p>Sin cobros registrados todavía</p></div>`;
      return;
    }
    const max = Math.max(...datos.map(d => d.total), 1);
    const fmt = n => '$' + parseFloat(n || 0).toLocaleString('es-AR', { minimumFractionDigits: 0 });
    el.innerHTML = `
      <div style="display:flex;align-items:flex-end;gap:0.5rem;height:140px;padding:0 0.25rem">
        ${datos.slice(-6).map(d => `
          <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:0.4rem;height:100%;justify-content:flex-end">
            <div style="font-size:0.65rem;color:var(--text-secondary);font-family:var(--font-data)">${fmt(d.total)}</div>
            <div style="width:100%;background:linear-gradient(180deg,var(--accent),var(--accent-dim));border-radius:4px 4px 0 0;height:${Math.max(8,(d.total/max)*90)}px"></div>
            <div style="font-size:0.68rem;color:var(--text-muted)">${d.mes}</div>
          </div>
        `).join('')}
      </div>`;
...
async function verDetallePrestamo(prestamoId, data) {
  ...
  const tasaMora = parseFloat(prestamo.TasaMora || 0);

  // Recalcular y persistir mora de cuotas vencidas antes de mostrar
  if (tasaMora > 0 && !SPREADSHEET_ID.includes('TU_SPREADSHEET_ID')) {
    setLoading(true, 'Actualizando mora...');
    try { await actualizarMoraPrestamo(prestamoId, tasaMora); } catch(e) { console.error(e); }
    setLoading(false);
  }

  // Releer cuotas frescas (con mora actualizada) si pudimos recalcular
  let cuotas = source.cuotas.filter(c => c.PrestamoID === prestamoId);
  if (tasaMora > 0 && !SPREADSHEET_ID.includes('TU_SPREADSHEET_ID')) {
    try {
      const cuotasFrescas = await getCuotas();
      cuotas = cuotasFrescas.filter(c => c.PrestamoID === prestamoId);
  ...
  // Calcular cuotas pendientes y su total (incluye mora)
  const pendientes      = cuotas.filter(c => c.Estado !== 'pagada');
  const totalPendiente  = pendientes.reduce((s, c) => s + parseFloat(c.MontoTotal || 0) + parseFloat(c.RecargoMora || 0), 0);
  const totalMora       = pendientes.reduce((s, c) => s + parseFloat(c.RecargoMora || 0), 0);
  const cuotasPagadas   = cuotas.filter(c => c.Estado === 'pagada').length;

  const cuotasHTML = cuotas.map(c => {
    const [d, m, y]  = c.FechaVencimiento.split('/');
    const fecha       = new Date(`${y}-${m}-${d}`);
    const mora        = parseFloat(c.RecargoMora || 0);
    let estadoClass   = 'cuota-pendiente';
    let estadoLabel   = 'Pendiente';
    let accion        = '';
    let montoMostrado = fmt(c.MontoTotal);

    if (c.Estado === 'pagada') {
      estadoClass = 'cuota-pagada';
      estadoLabel = `✓ ${c.FechaPago}`;
      if (mora > 0) montoMostrado = `${fmt(c.MontoTotal)} <span style="color:var(--danger);font-size:0.7rem">+${fmt(mora)} mora</span>`;
    } else if (fecha < hoy) {
      estadoClass = 'cuota-vencida';
      estadoLabel = mora > 0 ? `⚠️ Vencida` : '⚠️ Vencida';
      if (mora > 0) montoMostrado = `${fmt(c.MontoTotal)} <span style="color:var(--danger);font-size:0.7rem">+${fmt(mora)} mora</span>`;
  ...
  const btnCancelar = (prestamo.Estado === 'activo' && pendientes.length > 0) ? `
    <div style="margin-top:1rem;padding-top:1rem;border-top:1px solid var(--border)">
      <div style="font-size:0.78rem;color:var(--text-secondary);margin-bottom:0.5rem">
        ...
        ${totalMora > 0 ? ` <span style="color:var(--danger)">(incluye ${fmt(totalMora)} de mora)</span>` : ''}
      </div>
  ...
  document.getElementById('detail-info').innerHTML = `
    <div class="ganancia-preview">
      ...
      <div class="ganancia-row"><span>Tasa de interés</span><span>${prestamo.TasaInteres}%</span></div>
      ${tasaMora > 0 ? `<div class="ganancia-row"><span>Tasa de mora (mensual)</span><span style="color:var(--danger)">${tasaMora}%</span></div>` : ''}
...
function resetForm() {
  ['inp-monto','inp-tasa','inp-cuotas','inp-obs','inp-nombre','inp-dni','inp-tel','inp-dir','inp-mora'].forEach(id => {
...
async function submitPrestamo() {
  ...
  const tasaMora = parseFloat(document.getElementById('inp-mora').value) || 0;
  ...
  try {
    await guardarPrestamo({ clienteId, clienteNombre, monto, tasa, tasaMora, cuotas, fechaInicio: fecha, observaciones: obs });
